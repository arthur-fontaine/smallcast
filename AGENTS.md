...
**Smallcast targets one macOS — the current stable release — and nothing else.** macOS 26+, the Xcode 26
...
| Folder | Holds |
| --- | --- |
| `Smallcast/App/` | `@main`, `AppDelegate`, `AppCore` — the composition root |
| `Smallcast/DesignSystem/` | shared visual primitives; `Theme.swift` is the only design-token source |
| `Smallcast/Platform/` | system shims: `Permissions`, `AppPaths`, `Signposts`, `NotificationToken`, … |
| `Smallcast/Palette/` | the palette shell: panel, window controller, `RootPaletteView`, `PaletteScreen` |
| `Smallcast/Windows/` | the non-palette AppKit surfaces: `Dialog/`, `HUD/`, `About/`, `AppWindowController` |
| `Smallcast/Features/` | one folder per feature; larger ones split `Model/` `Service/` `UI/` `Settings/` |
...
- **Debug builds are their own channel** — `Smallcast Dev.app` / `com.smallcast.app.dev` — so a local run
...
- **XcodeGen owns the project.** `Smallcast.xcodeproj` is committed but generated from `project.yml`;
...
- `grep -rln 'import AppKit\|import SwiftUI\|import Cocoa' Smallcast/Features/*/Model/` returns nothing.
