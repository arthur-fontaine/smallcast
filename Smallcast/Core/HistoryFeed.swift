...
/// One row of the Recent list: something launched, or a calculation that was copied.
...
/// first. Deliberately chronological rather than ranked — this is "what did I just do", while
/// `AppIndex.matches` is "what am I looking for".
@MainActor
enum HistoryFeed {
    ...
    static func build(
        usage: UsageStore, apps: [AppEntry], calculations: [CalcHistoryEntry], query: String
    ) -> [HistoryItem] {
        let byKey = Dictionary(apps.map { ($0.usageKey, $0) }, uniquingKeysWith: { first, _ in first })
        // An app that's since been uninstalled (or a command from a disabled feature) simply drops out.
        var items = usage.records.compactMap { key, record -> HistoryItem? in
        ...
        if !q.isEmpty { items = items.filter { matches(q, $0) } }
    ...
    nonisolated private static func matches(_ query: String, _ item: HistoryItem) -> Bool {
        switch item {
        case .entry(let app, _):
            return AppIndex.score(query, for: app) != nil
