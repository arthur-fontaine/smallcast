...
private enum CapsLockRemap {
    ...
    private static let queue = DispatchQueue(label: "com.smallcast.capslock-remap", qos: .utility)
    ...
    private static func apply(_ mapping: String) {
        ...
        do {
            ...
            if process.terminationStatus != 0 {
                NSLog("Smallcast: hidutil remap exited %d", process.terminationStatus)
            }
        } catch {
            NSLog("Smallcast: hidutil caps lock remap failed: %@", error.localizedDescription)
        }
    }
}
