...
enum CalcPercent {
    static func evaluate(_ tokens: [CalcToken], query: String, rates: CurrencyRates) -> CalcResult? {
        parseOff(tokens, query: query, rates: rates)
            ?? parseAsPercentOf(tokens, query: query, rates: rates)
    }

    /// `<pct>% off <value>` → the value reduced by pct percent (`20% off 500` → 400).
    private static func parseOff(_ tokens: [CalcToken], query: String, rates: CurrencyRates)
        -> CalcResult?
    {
        guard let off = tokens.firstIndex(of: .ident("off")), off >= 2,
            tokens[off - 1] == .op("%"),
            let pct = CalcParser.evaluate(Array(tokens[0..<(off - 1)]), rates: rates),
            let base = CalcParser.evaluate(Array(tokens[(off + 1)...]), rates: rates)
        else { return nil }
    ...
    private static func parseAsPercentOf(_ tokens: [CalcToken], query: String, rates: CurrencyRates)
        -> CalcResult?
    {
        guard let asIdx = tokens.firstIndex(of: .ident("as")), asIdx + 2 < tokens.count,
            tokens[asIdx + 1] == .op("%"), tokens[asIdx + 2] == .ident("of"),
            let x = CalcParser.evaluate(Array(tokens[0..<asIdx]), rates: rates),
            let y = CalcParser.evaluate(Array(tokens[(asIdx + 3)...]), rates: rates), y != 0
        else { return nil }
        let ratio = x / y * 100
