...
/// Decrypts a Raycast `.rayconfig` export and maps the subset Smallcast supports. Format: gzip → JSON envelope → hex ciphertext decrypted with AES-256-GCM under a scrypt(N=16384,r=8,p=1) key → gunzip → settings JSON. Decrypt is CPU-heavy (scrypt) and pure Foundation/CryptoKit, so run it off the main actor.
enum RaycastImport {
    struct Result {
    ...
    /// Raycast `general.hyperKeyCode` → Smallcast physical key; unknown or absent values are skipped, and no value ever maps to `.none` (an export without a Hyper key must not disable one the user already configured).
    ...
    private static func mapSettings(_ json: [String: Any]) -> SettingsBackup.SettingsData? {
        ...
        // Exact-match only: a Raycast timeout outside Smallcast's option set is skipped, not clamped.
        ...
        // Raycast's window mode is a string ("compact"/"advanced"/…); Smallcast only has the compact toggle.
    ...
    /// Raycast marks favorited items with `favoriteOrder` (0-based). Only app favorites map to Smallcast, keyed by bundle ID (the same key `FavoritesStore` uses), preserving Raycast's order.
