...
enum Paster {
    ...
    /// Paste into `app` *without* activating it (⌘V delivered straight to its process), leaving Smallcast frontmost so the palette stays open. Returns whether content was written (and thus promoted).
    @MainActor @discardableResult
