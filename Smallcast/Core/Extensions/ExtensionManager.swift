...
/// extension screen.
@MainActor
final class ExtensionManager: ObservableObject, ExtensionRuntimeDelegate, ExtensionHostContext {
    @Published private(set) var installed: [InstalledExtension] = []
    @Published private(set) var state: ExtensionSessionState = .idle
    /// The command whose session is live, if any.
    @Published private(set) var running: ExtensionCommandRef?
    /// Toasts the running command asked for, newest last.
    @Published private(set) var toasts: [ExtensionToast] = []
    ...
    func start(appIndex: AppIndex, core: AppCore) {
    ...
    var pasteTarget: NSRunningApplication? { core?.extensionPasteTarget }
    ...
    func openPreferences(scope: String) {
        ...
        core?.showExtensionSettings(for: owner)
    }
    ...
    func present(toast: ExtensionToast) -> Int {
        ...
        guard core?.isPaletteVisible == true else {
            core?.showExtensionHUD([toast.title, toast.message].compactMap { $0 }.joined(separator: " — "))
            return stamped.id
        }
