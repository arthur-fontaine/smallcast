...
enum ShellCommandRunner {
    ...
    private static let queue = DispatchQueue(
        label: "com.smallcast.shell-command", qos: .userInitiated, attributes: .concurrent)
    ...
    nonisolated private static func execute(
        ...
        // Lets a shell config skip slow sections when Smallcast is the caller.
        process.environment = ProcessInfo.processInfo.environment.merging(["SMALLCAST": "1"]) { _, new in
            new
        }
    ...
    nonisolated private static func shellArguments(
        ...
        [loadingShellEnvironment ? "-ilc" : "-lc", command, "smallcast"] + arguments
    }
    ...
    private final class StreamCapture: @unchecked Sendable {
        ...
        static func make() -> StreamCapture? {
            let template = FileManager.default.temporaryDirectory
                .appendingPathComponent("smallcast-command-stream.XXXXXX").path
