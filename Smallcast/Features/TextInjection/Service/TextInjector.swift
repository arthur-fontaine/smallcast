...
final class TextInjector {
    ...
    /// A hotkey's target comes from `frontmostApplication`, which can be Smallcast itself.
    ...
    func copySelection(
        ...
        clipboardManager.prepareForSmallcastPasteboardMutation()
    ...
    private func restore(_ snapshot: PasteboardSnapshot, to pasteboard: any PasteboardAccess) {
        ...
        clipboardManager.synchronizeAfterSmallcastPasteboardMutation(
    ...
    private func finish(_ lease: TemporaryPasteboardLease) {
        ...
        case .restored(let changeCount):
            ...
            clipboardManager.synchronizeAfterSmallcastPasteboardMutation(changeCount: changeCount)
        case .superseded:
            break
        case .failed:
    ...
    private func tag(_ event: CGEvent) {
        event.setIntegerValueField(
            .eventSourceUserData,
            value: Paster.smallcastEventTag)
    }
