...
final class CurrencyRateStore {
    ...
    static let providerURL = URL(string: "https://raycast.com")!
    ...
    /// The newest snapshot, nil until the first one lands and always nil without consent.
    ...
    init() {
        ...
        // A disabled feature doesn't even read back a snapshot left on disk.
    ...
    /// No consent, no loop, so `AppCore.start()` can call this unconditionally.
    func start() {
