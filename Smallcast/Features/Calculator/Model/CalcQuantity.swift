...
enum CalcQuantity {
    ...
    private static func convertedResult(
        ...
        case .compound(let compound):
            return compoundResult(value.amount, unit: compound, expression: expression)
        case .currency(let definition):
            return currencyResult(value.amount, definition: definition, expression: expression)
        }
    }
    ...
    mutating func converted(
        ...
        case .compound(let from):
            guard let to = CalcUnits.byName[targetName] else {
                ...
                return fail(
                    "Cannot convert \(from.dimension.displayName) to "
                        + "\(CalcCurrency.categoryName).")
            }
            guard from.dimension == to.dimension else {
                return fail(
                    "Cannot convert \(from.dimension.displayName) to \(to.category.displayName).")
            }
