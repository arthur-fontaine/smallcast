...
/// Exponents over the fundamental dimensions the calculator models. Everything in `CalcUnits` maps
/// onto these, so arithmetic can combine units that have no table entry of their own: `km / h` is
/// length¹·time⁻¹ whether or not a "km/h" unit exists. Named `CalcDimension`, not `Dimension`,
/// because Foundation's Measurement API already owns that name.
struct CalcDimension: Equatable, Sendable {
    ...
    var angle = 0
    var temperature = 0
    /// Currency. Not a physical dimension, but it behaves like one for everything the calculator
    /// does: money adds to money, `$/h` is money¹·time⁻¹, and `$/€` cancels to a plain number.
    var money = 0
...
extension UnitDef {
    /// This unit's size in SI base units (m, kg, s, bit, rad, K) — or, for money, in the rate
    /// snapshot's base currency.
...
struct CompoundUnit: Equatable, Sendable {
    ...
    /// Every ISO code this unit is priced in, so the caller can check the snapshot quotes them all.
    var currencyCodes: [String] { terms.compactMap { $0.unit.currency } }
    ...
    /// "km/h" rather than in some canonical order. A term whose exponent cancels out is dropped,
    /// which is how `km / km` becomes a plain number.
