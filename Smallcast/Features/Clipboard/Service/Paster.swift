...
enum Paster {
    /// Stamped on Smallcast's own synthetic keystrokes so the snippet keyword tap can skip them.
    ...
    static func postCommandV(toPid pid: pid_t? = nil) {
        ...
        down.setIntegerValueField(.eventSourceUserData, value: smallcastEventTag)
        up.setIntegerValueField(.eventSourceUserData, value: smallcastEventTag)
