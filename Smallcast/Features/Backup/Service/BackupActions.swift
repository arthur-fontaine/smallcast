...
extension UTType {
    ...
    static let smallcastBackup = UTType(exportedAs: "com.smallcast.backup")
}
...
enum BackupActions {
    ...
    // MARK: - Smallcast native (own file panels; dialogs come from `AppCore`)
    ...
    static func chooseBackupFile() -> URL? { chooseFile(ofType: .smallcastBackup) }
    ...
    {
        guard let destination = chooseSaveLocation(named: "Smallcast", type: .smallcastBackup) else {
