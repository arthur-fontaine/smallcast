...
final class ExtensionNodeShims: @unchecked Sendable {
    ...
    private func feed(_ input: Data, to stdin: Pipe) {
        ...
        // A child that exits without reading everything must fail the write, not SIGPIPE Smallcast.
    ...
    /// `process.kill`, refusing every target that would signal Smallcast along with the child.
