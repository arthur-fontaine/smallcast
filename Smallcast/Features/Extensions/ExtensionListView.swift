...
struct ExtensionListView: View {
    ...
    private var rowList: some View {
        ScrollViewReader { proxy in
            ...
            .edgeDissolve()
            .thinScrollbar()
            .onChange(of: scrollToken) { scroll(proxy) }
            .onChange(of: selection) { scroll(proxy) }
        }
    }

    private func scroll(_ proxy: ScrollViewProxy) {
        guard screen.items.indices.contains(selection) else { return }
        proxy.scrollTo(screen.items[selection].id, anchor: .center)
    }
    ...
    private func accessoryView(_ fields: [String: RenderValue]) -> some View {
        HStack(spacing: Theme.Spacing.xs) {
            ...
            if let text = ExtensionAccessoriesView.label(fields["text"]) {
                Text(text)
                    .font(Theme.Typography.rowTrailing)
                    .foregroundStyle(ExtensionImage.color(fields["text"]?.objectValue?["color"]) ?? Theme.Colors.textSecondary)
                    .lineLimit(1)
            }
            ...
            ExtensionIconView(resolved: resolved, size: 56)
                ...
                .background(
                    RoundedRectangle(cornerRadius: Theme.Radius.menu, style: .continuous)
                        .fill(selected ? Theme.Colors.selection : (hovered ? Theme.Colors.rowHover : Color.white.opacity(0.03)))
                )
