...
Rule 1 also delivers the "works for windows Smallcast never moved" requirement: the capture happens in
...
- **The Dock ignores a gesture from a short-lived process.** The calls all report success and nothing
  happens. Smallcast is a resident menu-bar app, so this is free — but it is why a one-shot CLI cannot
...
5. Restore on a window Smallcast has never moved.
