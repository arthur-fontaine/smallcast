...
an independently selectable category. Smallcast reads `name`, `text`, and the optional `keyword` from
...
filename suffixes as snippets created in Smallcast, and duplicate keywords are preserved.
...
The preferred path is one atomic Accessibility replacement. Smallcast requires a focused element with
...
Some editors grant Accessibility but do not expose writable text attributes. In that case Smallcast
...
item has plain text that can be restored without another pasteboard write. Smallcast snapshots every
...
the Unicode-event fallback instead. The clipboard poller synchronizes to Smallcast's ownership changes
