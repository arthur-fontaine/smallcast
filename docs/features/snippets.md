...
Debug (`com.smallcast.app.dev`), beta, and stable therefore never share snippet files. The storage
...
frontmatter keeps the same identity. Renaming a file outside Smallcast appears as deletion of the old
...
an independently selectable category. Smallcast reads `title`, `text` and the optional `keyword` from
...
suffixes as snippets created in Smallcast, and duplicate keywords are preserved.
...
there are no aliases, so a key Smallcast does not know names itself in the error.
...
A token Smallcast cannot parse — an unknown name, an unknown modifier, a duplicated or unsupported
...
duplicates resolve by file identity. Smallcast-tagged synthetic events are ignored.
...
After either launcher or keyword delivery is confirmed, Smallcast may show a brief non-activating,
...
for permissions or activates Smallcast. Failed, cancelled, rejected, or prompt-cancelled expansions do
...
Longer or multiline fallback text uses a temporary paste. Smallcast snapshots every item, type and data
payload, then **lends a board holding nothing but the expansion** — one item carrying the plain text
and Smallcast's own marker type — and restores by rewriting the snapshot whole.
...
previous copy instead. A representation Smallcast cannot rewrite to mean the expansion is one it must
...
synchronizes to Smallcast's ownership changes so temporary or restored text is not added as new history.
...
any pasteboard restoration still owned by Smallcast.
...
non-ignored input to `cancelAutomaticExpansion`, and Smallcast's own tagged synthetic events classify
...
between revalidation and mutation, so Smallcast does not claim that impossible guarantee.
