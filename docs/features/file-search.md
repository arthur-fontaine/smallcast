...
- **Search is filename-only, and every list comes from Spotlight.** Smallcast creates no content index,
  ...
  `kMDItemFSContentChangeDate`, never from anything Smallcast recorded. The type filter narrows *which*
...
- **`~/Library` is never a scope Smallcast picks by itself.** A configured home root expands into its
...
- **Smallcast asks for no file permission.** Hidden metadata items and application bundles are filtered,
...
`com.smallcast.perf` signpost subsystem. `Tests/file-search-performance.swift` exercises the same service
...
half the folder hits on a developer machine are some `src` or `Smallcast`. The path itself is the preview's
...
ordinary settings carried by Smallcast settings backups; importing them grants no permission or
background access.
