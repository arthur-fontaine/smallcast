...
- **Search is filename-only and on demand.** An empty query does no work and Smallcast creates no
...
- Return calls `FileSearchCoordinator.open`, hides the palette without restoring focus, and uses the
  asynchronous `NSWorkspace` configuration API. A failure goes through Smallcast's dialog controller.
