...
- **Smallcast is the only writer.** There is no watcher and no revision check: a save replaces the file
...
- **The editor is the one surface snippets expand into.** `NoteTextView` adopts `InjectableTextView`,
  ...
  rather than posted as events at whichever app happens to be frontmost. Nothing else in Smallcast
...
restores the prior external application or Smallcast window and flushes without delaying the order-out —
...
Command-Q is bound to nothing app-wide, so no chord over Notes can quit Smallcast.
...
`NoteEditorInput.source` directly as `NSTextView.string` with one system font and Smallcast's note color.
...
state: editing the *active* note in another app while Smallcast has it open loses that edit the next time
