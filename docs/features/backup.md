...
Export and import of Smallcast's own data as a single `.smallcast` file, plus the entry point for
...
- **No absolute path may enter a `.smallcast`.** A clip's `imagePath` names a file on the Mac that wrote
...
| `Model/BackupArchive.swift` | Directory ⇄ `.smallcast`; the only file importing `AppleArchive` |
...
| `Service/RaycastImportReader.swift` | Raycast → Smallcast field mapping |
...
The old flat `Smallcast-Settings-*.json` export is gone rather than deprecated, and nothing reads it.
