...
all. `SMALLCAST=1` exists so an rc file can skip those sections: `[[ -n $SMALLCAST ]] && return`.
...
file reading `/dev/tty` can hang there but not for real users. There is **no timeout** — Smallcast never kills a
running command except through the output window's Stop button, and a command outlives Smallcast
quitting.
...
```
/bin/zsh -lc '<command>' smallcast <value1> <value2> …
```
...
- **rc-file noise is now visible.** With **Load shell environment** on, anything `~/.zshrc` writes
  reaches the log. The guard is the documented `[[ -n $SMALLCAST ]] && return`.
- **Stop is the one exception** to "Smallcast never kills a running command". Only the button does it;
...
user wrote themselves wants a deliberate second tap, not a red alarm. The gate is Smallcast's own
...
Smallcast dismisses an open palette before starting a custom command. With **Show output** off, a zero
exit status is silent; a launch failure or non-zero status opens a Smallcast dialog with the bounded
