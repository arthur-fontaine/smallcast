...
file reading `/dev/tty` can hang there but not for real users. There is **no timeout** — Smallcast
never kills a running command, and a command outlives Smallcast quitting.
...
Smallcast dismisses an open palette before starting a custom command. A zero exit status is silent; a
launch failure or non-zero status opens a Smallcast dialog with the bounded error detail. When the
