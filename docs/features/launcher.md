...
Stats, Smallcast itself, Mission Control, Siri, Time Machine, Screenshot, System Information, Font
...
`SystemActionCatalog` is a Foundation-only inventory of the macOS actions Smallcast exposes. Its
...
Toggle System Appearance changes macOS; Smallcast follows it only while its own Appearance is System.

Restart, Shut Down, Log Out, Empty Trash and Quit All Applications confirm before execution: ↵ runs
the action, Escape cancels. Every dialog is Smallcast's own: confirmations, failure reports and the Set
...
opened it. Volume and mute actions also show Smallcast's transient volume HUD, since macOS only draws
