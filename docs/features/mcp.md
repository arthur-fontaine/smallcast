...
Smallcast connects [Model Context Protocol](https://modelcontextprotocol.io) servers and offers their
tools to the model during a chat. A server is either a remote HTTP endpoint or a command Smallcast
runs on this Mac; either way it advertises tools, Smallcast namespaces them by the server's handle,
...
- **A dialog can grant a server, and only Settings can withhold one.** `MCPTrust` is `.ask` by
  default; the first call of a conversation goes through Smallcast's own three-way dialog. **Always
...
- **Smallcast exposes nothing back.** A server request — sampling, elicitation, roots — is declined
