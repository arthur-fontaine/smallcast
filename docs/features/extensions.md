...
- **Async** (`invoke`) for anything that needs the main actor — clipboard, toasts, window control,
  `fetch`, `exec`, `oauth`. Swift answers later through `__smallcast.settle`, so the JS thread never blocks on the
  UI.
...
- **Search-bar dropdown** — `List.Dropdown` and `Grid.Dropdown` draw as
  ...
  command's own dropdown answers before Smallcast's clipboard filter can. The list is
...
2. **Import from Raycast** — copies the already-built bundles out of a local Raycast. Nothing is
   ...
   so when Raycast has something Smallcast doesn't — installing in Raycast otherwise leaves no trace
...
a browser link, another app, a Shortcut — and `smallcast://` mirrors it so our own links never depend
...
tokens in the login Keychain (service `com.smallcast.extensions.oauth`, `kSecAttrAccessibleWhenUnlocked`),
scoped per extension and dropped on uninstall.

The redirect address belongs to the extension author's OAuth app registration, so Smallcast cannot choose
it — it can only be there to catch it. **Smallcast therefore claims `raycast`, `com.raycast` and `smallcast`
...
| --- | --- | --- |
| `App` | `raycast://oauth?package_name=Extension` | straight to Smallcast, no server |
| `AppURI` | `com.raycast:/oauth?package_name=Extension` | straight to Smallcast, no server |
| `Web` | `https://raycast.com/redirect?packageName=Extension` | through Raycast's page, which reopens a claimed scheme |

Claiming `raycast` means an installed Raycast competes with Smallcast for those links and macOS picks the
...
host rather than returning a wrong path. Node's `windows` override is absent: Smallcast only runs on
...
| OAuth tokens | macOS Keychain (`com.smallcast.extensions.oauth`) | yes |
...
**Builds happen in `$TMPDIR/smallcast-install-<UUID>/`**, named by `ExtensionCleanup.workspace` so the
