...
Smallcast has one app-wide provider layer for features that need text generation. AI Chat chooses its
...
- **Every request carries Smallcast's own preamble, and the user's text goes after it.**
  ...
  Smallcast over anything else. It is not shown in the pane, and `AIPreamble.swift` holds the only
...
- **Installed commands reuse their own login.** Smallcast launches the user's `codex`, `claude` or
  `opencode` executable without asking for or storing another key. Codex inherits the user's normal
  home and credential-store setting; Claude and OpenCode inherit their normal configuration. Smallcast
...
- **Claude and OpenCode are text transports, not agents.** Claude runs one turn with no tools, MCP
  ...
  deny-all permissions, disabled sharing and a private working directory; Smallcast deletes the session
...
manually. Smallcast does not ship or guess an API catalog that can become stale. Codex gets its models
...
the only destination Smallcast can offer the switch to honestly. Picking `None` sends
...
- Install and sign in to each supported command outside Smallcast, choose one of its discovered models,
...
commands are never installed by Smallcast; Settings links to their own install docs and offers a sign-in
...
bubble's status line. System messages become developer instructions alongside Smallcast's fixed
...
Smallcast's instructions and bounded conversation history as stdin, consumes newline-delimited JSON,
...
| OpenRouter | `plugins: [{id: "web"}]` — OpenRouter's own layer, any model | `image_url` part, only for models whose catalog lists the `image` modality | never yet — its catalog publishes a `file` modality Smallcast does not read | `tools` + `role: "tool"` turns |
...
`AIModelCapabilities.documents` is true only for the two HTTP shapes whose bodies Smallcast writes;
...
Settings → AI is a normal grouped `Form` inside Smallcast's existing Settings window. Its top AI
...
tools, run commands or touch files. That is a sandbox boundary on a local CLI, not Smallcast
