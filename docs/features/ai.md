...
Smallcast has one app-wide provider layer for features that need text generation. Settings chooses the
...
- **ChatGPT subscription access stays separate from API-key billing.** It uses `codex app-server` with
  a private `CODEX_HOME` under Smallcast's Application Support directory. Smallcast never reads browser
...
manually. Smallcast does not ship or guess a catalog that can become stale. ChatGPT models are different
...
login shell — and is never installed by Smallcast; Settings links to the install docs instead.
...
into Smallcast's private Codex home, never the user's `~/.codex`; the developer instructions say whether
...
Settings → AI is a normal grouped `Form` inside Smallcast's existing Settings window. It owns no
