...
- **A `.file` entry references the file where it lies and never copies it.** Its absolute path is
  ...
  Smallcast did not write. `kind` is a plain `TEXT` column, so the case cost no migration; an older
...
- **No recognition ever runs in the app process.** `ClipboardTextWorker` spawns one bundled
  ...
  allocations do not accumulate in Smallcast. The helper is handed a path and answers with text.
...
tree: a harness that touched `NSPasteboard.general` would land in the reader's own running Smallcast
...
`isCapturing` flag that `prepareForSmallcastPasteboardMutation` reads — so a paste Smallcast performs
...
A file copied in Finder is recorded as a reference, never as a copy: Smallcast writes nothing to
...
copy-only for the reverse reason: the path is the user's own file, and Smallcast must not move it.
