...
Smallcast checks GitHub Releases once a day, offers the newest release for its own channel in a native
...
- **Smallcast installs its own updates, and Homebrew stays out of the way.** Both casks declare
  `auto_updates true`, which is Homebrew's own flag for an app that manages its own version. `brew
  update && brew upgrade` therefore skips Smallcast entirely — it is never reported outdated, never
...
- **Nobody ever runs `xattr`.** An archive Smallcast fetched itself is not quarantined — macOS sets
  that flag for sandboxed downloaders and for apps that opt in with `LSFileQuarantineEnabled`, and
  Smallcast is neither. `Quarantine` checks anyway through `getxattr`/`removexattr` rather than the
  `xattr` tool, and an app that still carries the flag is refused rather than installed.
- **A matching signing identity is the only integrity guarantee.** Smallcast is self-signed and never
...
- **A build only ever updates within its own channel.** The channels are separate bundle ids installed
  side by side; crossing would mean installing a different app. `com.smallcast.app.dev` never updates
...
| --- | --- | --- |
| `com.smallcast.app` | `.stable` | releases |
| `com.smallcast.app.beta` | `.beta` | prereleases |
...
2. `ditto -x -k` it into a staging folder, and take whatever `.app` lands there — the bundle is named
   for its channel, so it is `Smallcast Beta.app` on beta.
...
and the cask installs, and `Smallcast-<version>.zip` for the updater. The zip is made with
