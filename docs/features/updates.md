...
and the cask installs, and `Smallcast-<version>.zip` for the updater. A stable run adds a
`Smallcast-Universal-<version>` pair from its `universal` job, uploaded second so the thin zip stays
