...
- **One funnel, whichever way an action started.** A shortcut and a launcher row both land on
  ...
  palette — once the palette is gone, the frontmost app is Smallcast, and the action would read its
...
- **Smallcast is never an event target.** `QuickActionRunner.selection(in:using:)` refuses our own
...
A custom action previews by default, switchable to Replace per row: Smallcast cannot know whether an
...
`HotKeyManager.start` can prune a binding whose action was deleted while Smallcast was off.
...
`QuickActionPanel` is Smallcast's **fourth borderless surface**, beside the dialog, the notes panel
...
Smallcast asserts, so it is the part worth checking by hand.
...
- Run one from the launcher (⌘Space → "Fix Grammar") with text selected behind it: the palette
  closes and the selection in the displaced app is what gets acted on, not Smallcast's own field.
...
- Press a shortcut with Smallcast's own Settings window frontmost: refused, with a HUD.
