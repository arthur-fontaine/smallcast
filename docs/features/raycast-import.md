...
Smallcast reads both `.rayconfig` formats in the wild. They come from different generations of a
...
Extensions → Export Settings & Data. **Smallcast never reads the keychain.** The user supplies the
passphrase in the same field v2 uses.

## v1 → Smallcast mapping
...
- **Clipboard records are flat** (`text` / `filePath` / `category`), not v2's nested representations,
  ...
  record can be any document and Smallcast has no kind for that, so its label imports as text.
...
returns Raycast's own values in a plain `RaycastV1Payload`; turning those into Smallcast's domain types
