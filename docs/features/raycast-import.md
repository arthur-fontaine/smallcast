...
Smallcast reads the `.rayconfig` written by Raycast 2.x: a `RAYCFG3` container holding an AES-256-GCM
...
those into Smallcast's domain types. That is the same pure-layer / platform-layer split
