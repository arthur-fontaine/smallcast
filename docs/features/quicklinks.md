...
Quicklinks reuse Smallcast's one template engine — the same
...
Smallcast's own dialog and leaves no partial state.
...
independently selectable category. Smallcast reads `name`, `link`, `createdAt` and the optional
