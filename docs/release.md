...
```sh
./Scripts/build-dmg.sh            # -> build/Smallcast-<version>.dmg (version from project.yml)
./Scripts/build-dmg.sh 0.5.7      # -> build/Smallcast-0.5.7.dmg
```
...
automatically; direct downloaders run `xattr -dr com.apple.quarantine "…/Smallcast.app"` once. Full
...
brew never reports Smallcast outdated, never re-downloads it, and never rolls a self-updated copy back.
...
- **channel** — `beta` or `stable`. Each builds a distinct app (`Smallcast Beta.app` / `Smallcast.app`)
  with its own bundle id, alongside the local `Smallcast Dev.app`. Beta gets an auto-incrementing
...
the docs section) and deploys it to GitHub Pages at `https://arthur-fontaine.github.io/smallcast/` on every
