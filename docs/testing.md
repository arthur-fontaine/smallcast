...
How to check that a change holds up. Smallcast has no XCTest target and no UI tests: the automated half
...
| Pure-layer purity | `grep -rln 'import AppKit\|import SwiftUI\|import Cocoa' Smallcast/Features/*/Model/` |
...
seconds to about 15. `SMALLCAST_TEST_JOBS=1` forces it back to one at a time. Each result is numbered
...
than `SMALLCAST_TEST_TIMEOUT` seconds (default 300) is killed and reported as timed out. Parallelism is safe
...
is the trap: a running Smallcast records every write to it as a genuine copy, so a fixture left there
...
| `voice-test` | `Voice/Model/` — the engine catalog's spellings, both tokenizers, `TransducerDecoder` |
| `voice-engine-test` | `Voice/Service/` — every downloaded engine on a spoken fixture; set `SMALLCAST_VOICE_FIXTURES="path=expected=lang;…"`, or it only compiles |
...
| `launch-history-test` | `Launcher/Model/LaunchHistoryStore.swift` — the Recent list's log |
...
| `hotkey-test` | `HotKeys/Model/DoubleTapModifier.swift`, `DoubleTapDetector.swift`, `HoldDetector.swift`, `HyperKey.swift`, `HotKeyAction.swift`, `Service/KeyShortcut.swift`, and the command→action mapping in `Launcher/Model/CommandID.swift` |
...
```sh
grep -rln 'import AppKit\|import SwiftUI\|import Cocoa' Smallcast/Features/*/Model/
```
...
xcodebuild build -project Smallcast.xcodeproj -scheme Smallcast -configuration Debug \
  CODE_SIGNING_ALLOWED=NO
xcodebuild build -project Smallcast.xcodeproj -scheme Smallcast -configuration Release \
  CODE_SIGNING_ALLOWED=NO
find ~/Library/Developer/Xcode/DerivedData -name "Smallcast*.app" -maxdepth 6 -print -quit
```
...
`Platform/Signposts.swift` emits eight intervals on the `com.smallcast.perf` subsystem: `AppCore.start`,
...
```sh
swiftc -O -swift-version 6 Smallcast/Platform/Signposts.swift \
...
```sh
swiftc -O -swift-version 6 Smallcast/Features/Calculator/Model/*.swift \
...
```sh
swiftc -O -swift-version 6 Smallcast/Platform/PasteboardFiles.swift \
    Smallcast/Features/Clipboard/Model/{ClipboardStore,ClipboardFilter,ColorValue,ColorFormat,ColorSpaces}.swift \
...
```sh
swiftc -O -swift-version 6 Smallcast/Features/Emoji/Model/{EmojiCatalog,EmojiData.generated}.swift \
    Smallcast/Features/Emoji/Service/{EmojiIndex,FrequentEmojiStore}.swift \
    Smallcast/Features/Launcher/Model/SearchRelevance.swift Smallcast/Platform/{AppPaths,Memo}.swift \
...
Run against the **Debug channel** (`Smallcast Dev.app`, `com.smallcast.app.dev`). It has its own prefs,
...
- Paste from clipboard history lands in that app, not in Smallcast
...
- ⌃⇧X asks first, through Smallcast's own dialog; Cancel and Esc both leave every entry in place
...
- Delete confirms through Smallcast, moves the file to Trash, and selecting another note never loses an
  unsaved edit
...
- Hiding restores the previous external app or Smallcast window
...
- Calendar in Menu Bar on Disabled: the calendar item is gone and Smallcast's own item is unaffected;
...
- Export produces a `.smallcast`; import applies it and reports a per-category summary
...
- Cancelling the save panel leaves nothing in `~/Library/Caches/com.smallcast.app.dev/backup-staging/`
...
```sh
rm -rf ~/Library/Caches/com.smallcast.app.dev
rm -rf "$HOME/Library/Application Support/com.smallcast.app.dev"
defaults delete com.smallcast.app.dev 2>/dev/null || true
tccutil reset Accessibility com.smallcast.app.dev 2>/dev/null || true
```
...
- Nothing was written outside `com.smallcast.app.dev/`. Channel isolation is not negotiable — a Dev build
