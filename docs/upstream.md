...
| **Popups open on a second display** — the Dock-icon flip happens before the window exists | — | `ActivationPolicy.windowWillOpen()` (the `.regular` flip, moved out of `windowDidOpen`) and its call in `AppWindowController.show` before `makeWindow`. Upstream flips after creating the window, and on macOS 26 a popup menu from that window can land on the wrong Space when the window is on a second display |
