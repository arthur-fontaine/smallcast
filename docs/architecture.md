...
How Smallcast is wired together. Per-feature internals live in [features/](README.md#features);
...
`CustomCommandArgumentSession`, `MeetingClock`), `NotesStore`, the twenty-one feature coordinators, and the
window controllers.
...
- **The main menu** — shaped by `SmallcastApp`'s `.commands`, which rebinds ⌘Q to Close Settings. It is
