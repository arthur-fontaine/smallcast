...
How code in Smallcast is written. This is **guidance** — it describes what the codebase already looks
...
one-line call into a layer nobody dares delete. Smallcast has no external API, no plugin surface and one
...
- **One folder per feature** under `Smallcast/Features/<Name>/`, holding everything that feature owns —
