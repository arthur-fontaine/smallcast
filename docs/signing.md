...
Smallcast is signed with a **stable self-signed identity** called `Smallcast Self-Signed`. Keeping the
...
## 1. Create the `Smallcast Self-Signed` identity (once)
...
security import /tmp/tc.p12 -k ~/Library/Keychains/login.keychain-db \
  -P smallcast -A -T /usr/bin/codesign
...
```sh
security find-identity -p codesigning | grep "Smallcast Self-Signed"
```
...
If you ever lose the secrets, just re-run this section — as long as the `Smallcast Self-Signed`
...
`Smallcast Dev.debug.dylib` is refused at launch because a self-signed identity carries no Team ID for
the loader to match. The flag is not part of the designated requirement, so turning it on costs no
Accessibility grant. Each entitlement in `Smallcast/Smallcast.entitlements` earns its place:
...
| `com.apple.security.personal-information.calendars` | `requestFullAccessToEvents()` returns `false` in milliseconds with no dialog, and Smallcast never appears under System Settings › Calendars |
...
to its entitlement, including resources Smallcast does not use. That grants nothing — only
`Smallcast.entitlements` does, and a row whose usage string `Info.plist` doesn't declare is skipped. It
...
`BundleSignature` already accepts a bundle signed by the Smallcast team under Apple's Developer ID
chain, even though releases are still signed with `Smallcast Self-Signed`. That is deliberate and
...
`Smallcast Self-Signed`, so a contributor keeps building with the one they created in §1 — same name,
their own key, never shared. Nothing about local development changes.

**Keep `Smallcast Self-Signed` in the login keychain after the switch.** It is the only way to ship a
