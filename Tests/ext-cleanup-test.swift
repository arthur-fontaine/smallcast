...
struct ExtensionCleanupTests {
    ...
    static func sweepTakesOnlyOurWorkspaces() {
        ...
        makeDirectory(roots.temp, "smallcast-install-ABC123")
        makeDirectory(roots.temp, "smallcast-install-DEF456")
        makeDirectory(roots.temp, "keep-me")
        makeFile(roots.temp, "unrelated.txt")

        ExtensionCleanup.sweepWorkspaces(in: roots.temp)

        expect(!exists(roots.temp, "smallcast-install-ABC123"), "a stale workspace is swept")
        expect(!exists(roots.temp, "smallcast-install-DEF456"), "every stale workspace is swept")
    ...
    static func cleanTakesOnlyOrphans() {
        ...
        makeDirectory(roots.temp, "smallcast-install-STRAY")
        ...
        expect(!exists(roots.temp, "smallcast-install-STRAY"), "a stray workspace goes")
