...
struct CustomCommandTests {
    ...
    static func main() async {
        ...
        setsid()
        let suiteName = "com.smallcast.custom-command-tests"
        ...
        let scriptDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("smallcast-scripts-\(UUID().uuidString)")
        ...
        let forwarded = await ShellCommandRunner.run(
            imported?.command ?? "", arguments: ["; touch /tmp/smallcast-import-should-not-exist"],
            workingDirectory: imported?.workingDirectory)
        check(
            "an imported script receives its argument as one inert word",
            forwarded.standardOutput == "; touch /tmp/smallcast-import-should-not-exist"
                && !FileManager.default.fileExists(atPath: "/tmp/smallcast-import-should-not-exist"))
        ...
        let marker = await ShellCommandRunner.run("test \"$SMALLCAST\" = 1")
        check("the SMALLCAST marker is exported so a shell config can detect us", marker.succeeded)
        ...
        let injected = await collect(
            ShellCommandRunner.stream(
                "printf '%s\\n' \"$1\"", arguments: ["; touch /tmp/smallcast-should-not-exist"]))
        check(
            "a value carrying shell syntax is data, not code",
            injected.log.contains("; touch /tmp/smallcast-should-not-exist")
                && !FileManager.default.fileExists(atPath: "/tmp/smallcast-should-not-exist"))
        ...
        let zdotdir = FileManager.default.temporaryDirectory
            .appendingPathComponent("smallcast-zdotdir-\(UUID().uuidString)")
        try? FileManager.default.createDirectory(at: zdotdir, withIntermediateDirectories: true)
        try? Data("alias smallcast_probe=true\n".utf8).write(
        ...
        let withEnvironment = await ShellCommandRunner.run(
            "smallcast_probe", loadingShellEnvironment: true)
        check(
        ...
        let withoutEnvironment = await ShellCommandRunner.run("smallcast_probe")
        check(
