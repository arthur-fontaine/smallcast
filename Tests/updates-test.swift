...
struct UpdatesTests {
    ...
    static func derivesChannels() {
        let stable = ReleaseChannel(bundleID: "com.smallcast.app")
        let beta = ReleaseChannel(bundleID: "com.smallcast.app.beta")
        let dev = ReleaseChannel(bundleID: "com.smallcast.app.dev")
    ...
    static func entry(
        tag: String, prerelease: Bool, draft: Bool = false, assets: [String] = ["Smallcast-x.zip"],
        body: String = "Notes."
    ) -> String {
        let list = assets.map {
            """
    ...
    static func rejectsUnusableFeeds() {
        ...
        expect(
            ReleaseFeed.newest(
                from: feed(entry(tag: "v0.3.0", prerelease: false, assets: ["Smallcast-x.dmg"])),
    ...
    static func picksTheZipThisMacCanRun() {
        let both = feed(
            entry(
                ...
                assets: ["Smallcast-0.3.0.zip", "Smallcast-Universal-0.3.0.zip"]))
        expect(
        ...
        let thinOnly = feed(entry(tag: "v0.3.0", prerelease: false, assets: ["Smallcast-0.3.0.zip"]))
        expect(
        ...
        let universalOnly = feed(
            entry(tag: "v0.3.0", prerelease: false, assets: ["Smallcast-Universal-0.3.0.zip"]))
        expect(
    ...
    static let composedBody = """
        ## What's Changed
        * Adjust top padding in **UpdateWindowView** by @arthur-fontaine in #304
            ...
            blocks.dropFirst().first
                == .bullet(
                    "Adjust top padding in **UpdateWindowView** by "
                        + "[@arthur-fontaine](https://github.com/arthur-fontaine) in "
    ...
    static func linksMentionsAndPullRequests() {
        ...
        expect(
            rendered("Fix by @arthur-fontaine in #304")
                == "Fix by [@arthur-fontaine](https://github.com/arthur-fontaine) in [#304](\(pull)/304)",
