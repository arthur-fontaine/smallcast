...
struct NotesTests {
    ...
    private static func testRepositoryAndSearch() throws {
        ...
        let support = root.appendingPathComponent("com.smallcast.app")
        ...
        let development = NotesRepository(
            applicationSupportDirectory: root.appendingPathComponent("com.smallcast.app.dev"))
        ...
        try stable.save(id: untitled.id, source: source)
        check(
            "Smallcast is the only writer, so a save replaces whatever is on disk",
        ...
        let empty = NotesRepository(
            applicationSupportDirectory: root.appendingPathComponent("com.smallcast.app.empty"))
    ...
    private static func temporaryRoot(_ name: String) -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent(
            "smallcast-notes-\(name)-\(UUID().uuidString)", isDirectory: true)
    }
