...
// Format detection, the v1 AES-256-CBC decrypt, the v2 RAYCFG3 container and the JSON → RaycastV1Payload mapping. No real export is used: the crypto cases build a synthetic fixture in-process and the mapping cases hand `payload(_:)` hand-written JSON. Turning payload values into Smallcast's own types lives in `RaycastImportV1` and `RaycastImportV2`, which need AppKit and are covered by the app build. Scrypt costs seconds per derive here, so `v2Fixture` derives once and only the cases that must reach key derivation do.
@main
@MainActor
enum RaycastTests {
