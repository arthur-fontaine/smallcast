...
struct MenuSearchTests {
    ...
    static func targetClassify() {
        ...
        expect(
            classify("Smallcast", isSelf: true, hasMenuBar: false) == .selfTarget,
            "self wins over the menu-bar check, since Smallcast itself runs accessory")
        expect(
        ...
        expect(
            classify("Smallcast", isSelf: true, isExcluded: true) == .selfTarget,
            "self still wins: Smallcast has no menu to exclude in the first place")
    }
