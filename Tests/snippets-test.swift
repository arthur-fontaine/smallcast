...
struct SnippetsTests {
    ...
    private static func testRaycastImport() {
        ...
        check(
            "Raycast import uses safe Smallcast defaults",
    ...
    private static func testRepositoryStorage() throws {
        ...
        let root = fm.temporaryDirectory.appendingPathComponent(
            "smallcast-snippets-tests-\(UUID().uuidString)",
            isDirectory: true)
        ...
        let stable = SnippetRepository(
            bundleIdentifier: "com.smallcast.app",
            applicationSupportRoot: channelRoot)
        let beta = SnippetRepository(
            bundleIdentifier: "com.smallcast.app.beta",
            applicationSupportRoot: channelRoot)
        let dev = SnippetRepository(
            bundleIdentifier: "com.smallcast.app.dev",
            applicationSupportRoot: channelRoot)
    ...
    private static func testRepositoryConcurrency() async throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory.appendingPathComponent(
            "smallcast-snippets-concurrency-\(UUID().uuidString)",
            isDirectory: true)
        try fm.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fm.removeItem(at: root) }
        ...
        boundaryEdit.text = "Smallcast edit"
        do {
    ...
    private static func testCopySelectionFallback() async {
        ...
        let backing = NSPasteboard(name: .init("smallcast-copy-tests-\(UUID().uuidString)"))
    ...
    private static func testDeliveryQueueAndPasteboard() async throws {
        ...
        let backingPasteboard = NSPasteboard(
            name: .init("smallcast-snippets-tests-\(UUID().uuidString)"))
        ...
        check(
            "pasteboard restoration leaves no Smallcast marker on the restored clipboard",
    ...
    private static func testStoreWatcher() async throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory.appendingPathComponent(
            "smallcast-snippets-watcher-\(UUID().uuidString)",
            isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let repository = SnippetRepository(
    ...
    private static func testDynamicPlaceholders() {
        ...
        // Raycast's snippet spelling resolves like Smallcast's.
    ...
    private static func testKeywordPolicy() {
        ...
        check("synthetic Smallcast events are classified as ignored", syntheticInput == .ignored)
...
final class ClipboardManager {
    static let internalType = NSPasteboard.PasteboardType("com.smallcast.internal")
    func prepareForSmallcastPasteboardMutation() {}
    func synchronizeAfterSmallcastPasteboardMutation(changeCount: Int) {}
}
...
enum Paster {
    static let smallcastEventTag: Int64 = 0x54494E59
