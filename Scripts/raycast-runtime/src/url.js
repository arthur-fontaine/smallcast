...
// Smallcast runs only on macOS, so Node's Windows path override is deliberately out of scope.
export function fileURLToPath(input, options = {}) {
  ...
  if (options?.windows) {
    throw new Error("Windows file paths are not supported in Smallcast extensions.");
  }
